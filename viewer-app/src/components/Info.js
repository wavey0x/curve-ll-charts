...
const Info = () => {
    ...
    const formatHours = (seconds) => (seconds ? (seconds / 3600).toFixed(2) : '0');
            ...
            <table className="data-table">
                ...
                <tbody>
                    {rows.map((row, index) => (
                        <tr key={index}>
                            <td>{row.symbol}</td>
                            <td className={getBoldClass(row['30'] === highest30)}>{formatPercentage(row['30'])}</td>
                            <td className={getBoldClass(row['60'] === highest60)}>{formatPercentage(row['60'])}</td>
                            <td className={getBoldClass(row['90'] === highest90)}>{formatPercentage(row['90'])}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
            ...
            <table className="data-table">
                ...
                <tbody>
                    {rows.map((row, index) => (
                        <tr key={index}>
                            <td>{row.symbol}</td>
                            <td className={getBoldClass(row.tvl === highestTvl)}>{formatCurrency(row.tvl)}</td>
                            <td>{formatHours(row.profit_unlock_period)}</td>
                            <td className={getBoldClass(row.fee === lowestFee)}>{formatPercentage(row.fee / 100)}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
